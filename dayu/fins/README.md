...
`CnPipeline` 当前承担 A 股下载、CN/HK 上传与 CN/HK process。A 股下载链路已按 downloader / workflow / storage commit 拆分：
...
CN download 的 source 写入顺序固定为：先通过 source 仓储创建或更新 meta，再通过 blob 仓储写 PDF / Docling JSON，最后通过 source 仓储提交 `file_entries`、`primary_document` 与完成态 meta；不得绕开 source 仓储直接刷新 manifest。`overwrite=True` 与 SEC 对齐，走 filing maintenance 仓储的 ticker 级 `clear_filing_documents(ticker)`，不会复用 staged 或完成态文件。披露易 downloader 已按同一 `CnReportDiscoveryClientProtocol` 实现；HK 默认 pipeline 装配仍保持显式未接入，接入后会沿用同一 workflow，并把 HK Q1/Q3 查无收口为 skipped。
