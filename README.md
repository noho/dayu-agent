...
- `--ticker` 支持 `0700.HK` / `HK.00700` / `600519.SH` / `sh600519` / `AAPL.US` / `BRK-A` 等常见变形，内部统一归一化到裸码（港 4 位补零、沪深 6 位、美股类股分隔符统一为点号）。`BRK` / `BRK.A` / `BRK.B` 会归一到 `BRK.B`，避免同一 SEC issuer 的财报写入不同目录。公司名仍可作为 ticker 传入，由仓储 alias 查表兜底。
