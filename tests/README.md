...
- `test_streamlit_chat_utils.py` 负责守住 Streamlit Chat 纯函数与流式运行时边界：`chat/utils.py` 的 session/trace 标识生成、流式文本提取与 Markdown 归一、事件折叠与副作用判定，以及 `chat_tab.py` / `chat/stream_runtime.py` 中消息与帧状态 dataclass 的稳定语义不能漂移。
