...
- `tests/fins/`
  ...
  - `tests/fins/test_cninfo_downloader.py` 负责守住巨潮 discovery 的网络隔离与候选去重边界：测试必须通过 `httpx.MockTransport` 注入全市场 `szse_stock.json` 与 `hisAnnouncement/query` fixture，不访问真实巨潮；公司解析 fixture 必须覆盖深市、沪市与全文搜索噪声不影响精确 ticker 解析；候选 fixture 必须覆盖 `secCode` 与 `adjunctType=PDF` 过滤，并守住摘要、英文版/英文简版、港股公告、关于财报正本的公告类材料不会进入正本候选；报告去重键必须是 `(fiscal_year, fiscal_period)`，同一年同 period 才按更正版优先、披露日期最新选择，不能把不同 fiscal_year 的年报互相吞掉
  ...
  - 其中 `tests/fins/test_score_docling_ci.py` 负责守住 CN/HK Docling scorer 的最小闭环：样本发现必须通过 source/processed 仓储对齐 active filing/material，snapshot 读取必须通过 blob 仓储，坏 `tool_snapshot_meta.json` 进入 completeness hard gate，正文表格占位符必须能解引用到现有 `list_tables/get_table`，并且 JSON/MD 报告保持可供后续 CI runner 复用的结构
