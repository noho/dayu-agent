...
- `test_cli_interactive_coverage.py` 这类 CLI UI 测试里的 fake session 应直接实现 `ChatServiceProtocol` / `PromptServiceProtocol` 公开方法（如 `submit_turn`、`resume_pending_turn`、`list_resumable_pending_turns`、`cleanup_stale_pending_turns`、`list_session_recent_turns`、`clear_session`、`submit`），不要继续只靠旧的 `stream/stream_turn` 形态再由测试把对象硬塞进 `interactive_ui`。
