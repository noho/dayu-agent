...
- `tests/fins/`
  ...
  - `tests/fins/test_cninfo_downloader.py` 负责守住巨潮 discovery 的网络隔离与候选去重边界：测试必须通过 `httpx.MockTransport` 注入 fixture，不访问真实巨潮；报告去重键必须是 `(fiscal_year, fiscal_period)`，同一年同 period 才按更正版优先、披露日期最新选择，不能把不同 fiscal_year 的年报互相吞掉
  ...
  - `tests/fins/test_cn_download_workflow.py` 负责守住 CN/HK 下载 workflow 的仓储顺序与恢复语义：fake downloader 不写 workspace，PDF / Docling JSON 必须经 blob 仓储写入，source meta 必须经 source 仓储 create/update 提交；fast skip、PDF SHA skip、staged PDF/Docling 恢复和 overwrite ticker 级清理都要用文件系统仓储 fixture 验证
