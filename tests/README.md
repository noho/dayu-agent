...
- `tests/fins/`
  ...
  - `tests/fins/test_cn_download_workflow.py` 负责守住 A 股下载 workflow 的仓储顺序与恢复语义：fake downloader 不写 workspace，PDF / Docling JSON 必须经 blob 仓储写入，source meta 必须经 source 仓储 create/update 提交；fast skip、PDF SHA skip、staged PDF/Docling 恢复和 overwrite ticker 级清理都要用文件系统仓储 fixture 验证
